...
for genome in prj.genomes.__dict__.keys():
    genome = getattr(prj.genomes, "human")
    output_dir = os.path.join(prj.paths.output_dir, "spiked_genomes")

    spiked_dir = os.path.join(output_dir, genome)

    for _dir in [prj.paths.output_dir, output_dir, spiked_dir]:
    ...
    annotation = os.path.join(prj.paths.output_dir, "metadata", "guide_annotation.csv")
    df = pd.read_csv(annotation)

    write_annotation(df, prj.config, output_fasta, output_gtf)

    # Make STAR index and supporting files
    if genome == "hg38_spiked":
        # Get fasta genome
        os.system(
